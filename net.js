...
(function () {
  ...
  // Yazılan ad burada saklanır. Rol seçme ekranı lobiyi baştan çizdiği için
  // isim kutusu kayboluyor, ad boş gidiyor ve sunucu reddediyordu.
  let ADIM = '';
  ...
  // Adını yazmadan masaya oturulmaz.
  function adTamam() {
    const ad = nameVal();
    if (ad.length < 2) {
      note('Önce adını yaz — masada bu adla görüneceksin.');
  ...
  function rolSec(code, o) {
    ...
    lobbyHTML(
      ...
      '</div>' +
      '<div style="font-size:11px;letter-spacing:.14em;text-transform:uppercase;opacity:.5;margin-bottom:4px">adın</div>' +
      '<input id="lbName" value="' + ADIM.replace(/"/g, '&quot;') + '" maxlength="12" style="' + INP + ';margin-bottom:10px">' +
  ...
  // Tek kişilik oyunda "bu taş işliyor" uyarısı motorun confirm'iyle çıkıyor.
  // Çok oyunculuda motor SUNUCUDA çalıştığı için o uyarı oyuncuya hiç
  // ulaşmıyordu: 71 ceza yiyor ama sebebini atarken görmüyordu.
  function islekUyari(id) {
    const el = (api.S.players[SEAT] || {}).hand || [];
    if (el.length <= 1) return true;              // son taş: bitiriyorsun, ceza yok
    const t = el.find(x => x.id === id);
    if (!t || api.isWild(t) || !api.isLive(t)) return true;
    return confirm('Bu taş yerdeki perlere işliyor.\n\nAtarsan 71 ceza yazarsın ve taş ölür — kimse alamaz.\n\nYine de atılsın mı?');
  }
